...
class Pipeline:
    def __init__(self, regions: List[str] = REGIONS,
                 start_year: int = 1990,
                 end_year: int = 2023,
        ...
        self.acq = DataAcquisition(cds_url=CDS_API_URL, cds_key=CDS_API_KEY, estat_key=ESTAT_API_KEY)
        self.prep = DataPreprocessor(raw_dir, processed_dir)
        self.gen = RawDataGenerator(raw_dir, processed_dir)
        
    def run(self, skip_climate_acquisition: bool = False,
            skip_yield_acquisition: bool = False, 
            skip_raw_generation: bool = False) -> Dict:
        """
        Run pipeline
        ...
        if not skip_yield_acquisition:
            ...
            for year in range(self.start_year, self.end_year+1):
                try:
                    ...
                    if stats_data_id:
                        municipality_code_list = [MUNICIPALITY_CODES[region] for region in self.regions if region in MUNICIPALITY_CODES]
                        ...
                        if data_inf:
                            for item in data_inf:
                                ...
                                region = next((r for r, code in MUNICIPALITY_CODES.items() if code == municipality_code), None)
                                
                                if region and region in self.regions:
                                    try:
                                        value_float = float(value) if value else None
                                        yield_data_by_region[region].append({'year': year, 'yield': value_float})
                                    
                                    except (ValueError, TypeError):
                                        logger.warning(f'Invalid yield value for {region} in year {year}: {value}')
            ...
            for region, data_list in yield_data_by_region.items():
                if data_list:
                    ...
                    yield_df.to_csv(os.path.join(self.raw_dir, f'{region}_yields.csv'), index=False)
        ...
        if not skip_raw_generation:
            ...
            for region in self.regions:
                try:
                    ...
                    result['raw_generation'][f'{region}_climate_df'] = f'error: {str(e)}'
        ...
        try:
            ...
            result['preprocessing'] = {
                ...
                'columns': joined_data.columns.tolist() if not joined_data.empty else []
            }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_year', type=int, default=1990, help='Data acquisition start year')
    parser.add_argument('--end_year', type=int, default=2023, help='Data acquisition end year')
    parser.add_argument('--skip_raw_generation', action='store_true', help='Skip raw data generation')
    parser.add_argument('--skip_yield', action='store_true', help='Skip yield data acquisition')
    parser.add_argument('--skip_climate', action='store_true', help='Skip climate data acquisition')
