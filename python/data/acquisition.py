...
class DataAcquisition:
    def __init__(self, cds_url: str, cds_key: str, estat_key: str):
        self.url = cds_url or os.getenv('CDS_API_URL', 'https://cds.climate.copernicus.eu/api')
    ...
    def find_stats_data_id(self, year_str: str):
        ...
        endpoint = f'http://api-esta.go.jp/rest{ESTAT_API_VERSION}/app/json/getStatsList'
        params = {
        ...
        if len(self.estat_key) > 8: masked_params['appId'] = self.estat_key[:4] + '...' + self.estat_key[-4:]
        else: masked_params['appId'] = '********'
        ...
        try:
            ...
            if status == 0:
                ...
                if isinstance(table_inf_list, dict): table_inf_list = [table_inf_list]
                
                if not table_inf_list:
                    print(f'  -> Result: Could not find any stat data for{year_str}. Please try another year.')
                    return None
                
                print(f'  -> Result: Found {len(table_inf_list)} stats data for{year_str}.')
                ...
                for table in table_inf_list:
                    ...
                    if title and '収穫量' in str(title):
                        ...
                        print(f'  /> Options: ID={selected_table.get('@id')}, Title={title}')
                        break
                    
                if not selected_table and table_inf_list:
                    selected_table = table_inf_list[0]
                    title_obj = selected_table.get('TITLE', {})
                    title = title_obj.get('$') if isinstance(title_obj, dict) else selected_table.get('TITLE')
                    print(f"  -> Selected (first option): ID={selected_table.get('@id')}, Title={title}")

                if selected_table:
                     print(f"  -> Selected stats data for {selected_table.get('@id')}")
                     return selected_table.get('@id')
                else:
                     print("  -> Error: found candidate but could not find stats data ID.")
                     return None
                
            elif status == 1:
                print(f"  -> Result: could not find stats data for {year_str} (no data).")
                return None
            else:
    ...
    def get_crop_data(self, stats_data_id: str, municipality_codes: str, indicator_code: str):
        """
        ...
        """
        endpoint = f"https://api.e-stat.go.jp/rest/{ESTAT_API_VERSION}/app/json/getStatsData"
        params = {
            'appId': self.estat_key,
        ...
        try:
            ...
            if status == 0:
                ...
                if not data_inf:
                     ...
                     if isinstance(data_inf, dict): data_inf = [data_inf] # 1件の場合
                     for i, item in enumerate(data_inf[:5]):
                          ...
                          print(f"    {i+1}: Cities={municipality_code}, Time={time_code}, Value={value} {unit or ''}", end="")
                          if annotation: print(f" (注釈:{annotation})", end="")
                          print()
                     if len(data_inf) > 5: print("    ...")
